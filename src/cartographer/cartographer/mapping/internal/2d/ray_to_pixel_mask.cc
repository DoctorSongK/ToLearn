...
// core: bresenham绘图方法
// https://blog.csdn.net/chaosir1991/article/details/109561010 实际原理可参考该链接
std::vector<Eigen::Array2i> RayToPixelMask(const Eigen::Array2i& scaled_begin,
  ...
  // x相等, 斜率不存在的情况，在这里无论是正无穷大还是负无穷大斜率，均按照正无穷大完成pixel压入
  /**       ^b
   *     ^  |                ^
   *     |  |a               |
   *     |                   |------   ^
   *     |————————           |  |a     | 与正无穷的处理方式相同，仅用来将栅格给遍历进容器
   *     |                      |  ->  |
   *                            V b
   *   (斜率为正无穷大)       (斜率为负无穷大)
   */ 
  ...
  // dy > 0 时的情况，上半部分
  sub_y += dy * first_pixel;   // 获得起点在栅格中的位置
  if (dy > 0) {
    while (true) {
      if (!isEqual(pixel_mask.back(), current)) pixel_mask.push_back(current);   // 加入右侧点（在相等特殊情况下加入右上点）
      while (sub_y > denominator) {
        sub_y -= denominator;
        ++current.y();
        if (!isEqual(pixel_mask.back(), current)) pixel_mask.push_back(current);  // 加入右上点
      }
      ++current.x();
      ...
      sub_y += dy * 2 * subpixel_scale; // x增加一个栅格，对应sub_y增加一个斜率
    }
    // Move from the pixel border on the right to 'scaled_end'.
    sub_y += dy * last_pixel;   // 获得终点在栅格中精确位置，并绘制
    if (!isEqual(pixel_mask.back(), current)) pixel_mask.push_back(current);
    while (sub_y > denominator) {
      sub_y -= denominator;
      ++current.y();
