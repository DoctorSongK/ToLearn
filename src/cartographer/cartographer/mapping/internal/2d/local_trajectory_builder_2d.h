...
/**
 * @brief 整个前端的执行，包含传感器输入，姿势外插器，scan_match，CSM，地图生成等任务
 * 
 */
class LocalTrajectoryBuilder2D {
 public:
  /**
   * @brief 将点云插入到地图后的result
   * @param constant_data 包含重力校正量、原点处点云
   * @param insertion_submaps active_map中的子图
   */
  struct InsertionResult {
    ...
    /**
   * @brief 扫描匹配的result
   * @param local_pose 前端匹配位姿
   * @param range_data_in_local 经过扫描匹配之后位姿校准之后的雷达数据
   * @param insertion_result 子图、点云等
   */
 ...
 private:
  ...
  // QUES: 已知动态滑窗子图仅能包含两个子图，那其他构建的子图存放到哪啦？？？
  ActiveSubmaps2D active_submaps_;                                          // 动态滑窗子图
  ...
  std::unique_ptr<PoseExtrapolator> extrapolator_;                          // 姿势外插器

  int num_accumulated_ = 0;                                                 // 可决定去畸变后有效帧插入频率
  sensor::RangeData accumulated_range_data_;                                // 去畸变后点云存储容器
