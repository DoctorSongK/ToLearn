...
// NOTE: RealTimeCorrelativeScanMatcher2D::Match
// 是通过对像素坐标进行遍历，所以这个函数计算出的坐标的分辨率是栅格的分辨率，不是连续的
//       CeresScanMatcher2D::Match
//       优化的是物理坐标，不是像素坐标，所以坐标是连续的
  ...
  class GridArrayAdapter {
   public:
    ...
    void GetValue(const int row, const int column, double* const value) const {
      // 处于地图外部时, 赋予最大free值
      // 在这里可以看做把地图裹在了kPadding中
      /**
       *       __________________
       *      |     kPadding     |
       *      |     ————————     |
       *      |    |        |    |
       *      |    | origin |    |   四周被kPadding所填充
       *      |    |        |    |
       *      |     ————————     |
       *      |__________________|
       *
       */
      // 关于传入经处理后的row是在interpolator.Evaluate传入的，也就导致kPadding内无东西
...
// 因为点云数量是不确定的所以这边residuals用的DYNAMIC，也有可能因为这里用的动态值，所以后面给出了残差数量（point_cloud.size()）
