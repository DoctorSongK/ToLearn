...
SearchParameters::SearchParameters(const double linear_search_window,
  ...
  // NOTE: 可参考https://zhuanlan.zhihu.com/p/398383561公式6、7
  // 下列公式原理要求：寻找最远的击中点，在旋转一个分辨角度的情况下，其末端刚好移动一个栅格分辨率，以免旋转一个分辨角，丢掉一些栅格点
  // 公式：
  //       dmax = max ||hk||
  //       delta = arccos(1-r*r/(2*dmax*dmax))    -->根据余弦公式求等腰三角型（长为dmax，底边为r）夹角
...
// QUES: 感觉这里有点问题，个人理解是在不超出地图边界的情况下，点云可移动范围
