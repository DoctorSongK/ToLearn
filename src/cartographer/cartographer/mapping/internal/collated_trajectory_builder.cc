...
CollatedTrajectoryBuilder::CollatedTrajectoryBuilder(
    ...
    : sensor_collator_(sensor_collator),
      ...
      // param: 对应trajectory_builder.lua文件中collate_landmarks参数，若为false，不将landmark数据放入到阻塞队列中
      collate_landmarks_(trajectory_options.collate_landmarks()),
      // collate_fixed_frame 为 true, 将gps数据放入阻塞队列中
      // param: 对应trajectory_builder.lua文件中collate_fixed_frame参数，若为true，将gps数据放入阻塞队列中
  ...
  // sensor::Collator的初始化
  // 完成分发队列的填充
  // core: 传感器数据处理流程 step3->利用collator.c中AddTrajectory函数根据topic不同登记传感器处理函数HandleCollatedSensorData函数，用于分发队列按时间戳处理数据进行回调
 ...
 */
// core: 传感器数据处理流程 step4->传感器处理函数，待多传感器时间同步后将所处理数据再传输至global_trajectory_builder进行处理
void CollatedTrajectoryBuilder::HandleCollatedSensorData(
  ...
  // 按照传感器log频率kSensorDataRatesLoggingPeriodSeconds完成如下信息格式输出
