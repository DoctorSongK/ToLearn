...
class GlobalTrajectoryBuilder : public mapping::TrajectoryBuilderInterface {
 public:
  ...
  // core: 传感器数据流向->step6,此时数据流向有两个，一个是前端local_trajectory_builder，一个是后端pose_graph_(注意激光并不一样)
  ...
  // note: 加入到后端之前, 先做一个距离的计算, 只有时间,移动距离,角度 变换量大于阈值才加入到后端中
  void AddSensorData(const std::string& sensor_id,
  ...
  // QUES: 建图的时候似乎并没有用到这个地方，而且没找到调用的接口，按理讲应该是在sensor_bridge外部调用。
