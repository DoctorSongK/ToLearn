...
// 跟踪轨迹之间连接状态的类。与ConnectedComponents相比，它还跟踪全局约束最后一次连接到轨迹的时间。
// 这个类是线程兼容的
// 描述不同轨迹之间的连接状态
class TrajectoryConnectivityState {
 public:
  ...
  // Add a trajectory which is initially connected to only itself. 添加最初仅与自身连接的轨迹。
  ...
  // connected time.
  // 连接两条轨迹。如果任一轨迹未被跟踪，则会被跟踪。这个函数对其参数的顺序是不变的。重复调用Connect会增加连接计数并更新上次连接时间。
  ...
  // 确定两个轨迹是否已（过渡）连接。如果任一轨迹未被跟踪，则返回false，除非是同一轨迹，否则返回true。这个函数对其参数的顺序是不变的。
  bool TransitivelyConnected(int trajectory_id_a, int trajectory_id_b) const;

  // The trajectory IDs, grouped by connectivity. trajectory_ids，按照连接性分组
