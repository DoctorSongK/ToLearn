...
/// @brief  包含往pose_graph中设置初始位姿，加载各项传感器位姿、landmark等信息，向submap中加载node
class PoseGraph : public PoseGraphInterface {
 public:
  ...
  /**
   * @brief Set the Initial Trajectory Pose object
   *        在时间“time”下设置“from_trajectory_id”相对于“to_trajectory_id”的相对初始位姿“relative_pose”
   * @param[in] from_trajectory_id 
   * @param[in] to_trajectory_id 
   * @param[in] pose 
   * @param[in] time 
   */
