...
// 不表达实际意思 在参数列表后添加 -> decltype( ), 这是后置返回类型,
// 代表返回的类型是 () 中的类型 在 C++14 中, 则可以不用 decltype
...
// 由于是全局函数的重载，所以这里需要将os作为左操作数参数显式指定
...
struct SubmapId {
  ...
  bool operator<(const SubmapId& other) const {
    // c++11: std::forward_as_tuple() 用于接受右值引用数据生成tuple
    return std::forward_as_tuple(trajectory_id, submap_index) <
           std::forward_as_tuple(other.trajectory_id, other.submap_index);
  }
...
class MapById {
 ...
 public:
  ...
  class ConstIterator {
   public:
    ...
    // c++11: lower_bound 用于在指定区域内查找不小于目标值的第一个元素，如果没有找到的话应该返回end()
    explicit ConstIterator(const MapById& map_by_id, const int trajectory_id)
      ...
      if (current_trajectory_ != end_trajectory_) {
        ...
        // 如果数据不可用就将current_data_设置为下一条轨迹的开始
  ...
  ConstIterator lower_bound(const int trajectory_id,
    ...
    // 二分查找
    // c++11: std::prev() std::next() 用来获取一直距离指定迭代器n个元素的迭代器，默认移动距离为1（前者往前找 后者往后找，但是不局限方向，前者方法也可向后查询）
    ...
    while (left != right) {
      ...
      // 防止经舍入后的二分中心大于了原有二分中心点，若满足上述猜想，则std::prev 1 个距离
 ...
 private:
  /// @brief 索引包含信息：1、是否可以追加（不明白干啥的）
  /// 2、存储NodeId或者SubmapId的index以及该索引处实际存储的数据
  struct MapByIndex {
