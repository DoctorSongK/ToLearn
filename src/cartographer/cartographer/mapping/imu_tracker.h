...
// core: imu追踪器，只负责旋转量的跟踪与维护，在cartographer中并不对IMU的translation进行推算（重要特性）；
// 均匀的速度被假定为激光帧之间的线性速度，而odometry data只使用最新的odom topic数据和最新激光帧数据时间戳
// 前一个odom topic数据来估算平均线速度。
/**
 * @brief 
...
class ImuTracker {
 ...
 private:
  ...
  common::Time time_;                                           // 上一时刻预测位姿时间
  common::Time last_linear_acceleration_time_;
  Eigen::Quaterniond orientation_;                              // 历史姿态
  Eigen::Vector3d gravity_vector_;                              // 重力加速度方向
