...
class MapBuilder : public MapBuilderInterface {
 ...
 private:
  const proto::MapBuilderOptions options_;                              // MapBuilder相关参数
  common::ThreadPool thread_pool_;                                      // 线程池

  std::unique_ptr<PoseGraph> pose_graph_;                               // 后端优化器（抽象类）

  std::unique_ptr<sensor::CollatorInterface> sensor_collator_;          // 传感器同步分发器
  std::vector<std::unique_ptr<mapping::TrajectoryBuilderInterface>>
      trajectory_builders_;                                             // 轨迹构造器
