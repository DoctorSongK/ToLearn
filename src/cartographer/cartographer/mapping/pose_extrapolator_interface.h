...
class PoseExtrapolatorInterface {
 public:
  ...
  struct ExtrapolationResult {
    ...
    std::vector<transform::Rigid3f> previous_poses;         // 先前所有的位姿（0 - N-1）
    // The pose for the requested time at index N.
    transform::Rigid3d current_pose;                        // 当前时刻的位姿
    Eigen::Vector3d current_velocity;                       // 当前时刻的线速度
    Eigen::Quaterniond gravity_from_tracking;               // 当前时刻线速度与当前重力方向夹角
  };
