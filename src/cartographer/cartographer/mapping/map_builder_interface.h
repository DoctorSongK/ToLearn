...
// 此接口用于库和RPC实现。实现将整个SLAM堆栈连接起来（抽象类）。
class MapBuilderInterface {
 public:
  ...
  // trajectory_ids.
  /**
   * @brief 从给定proto文件中加载SLAM状态，返回新trajectory_id的重新映射
   * 
   * @param[in] reader 
  ...
  // trajectory_ids.
  /**
   * @brief 从pbstream文件中加载SLAM状态，返回新trajectory_id的重新映射
   * 
   * @param[in] filename 
   * @param[in] load_frozen_state 
   * @return std::map<int /* trajectory id in proto , int /* trajectory id > 
