...
void GrowAsNeeded(const sensor::RangeData& range_data,
  ...
  // 找到点云的bounding_box
  // Eigen::vector::head(n)       获取向量的前n个元素
  // Eigen::vector::tail(n)       获取向量尾部的n个元素
  // Eigen::vector::segment(i,n)  获取从向量的第i个元素开始的n个元素
  ...
  // 是否对地图进行扩张
  // 注意这里的扩张并不是遍历所有的点，而是看看最大索引、最小索引包含在地图内部吗
...
void CastRays(const sensor::RangeData& range_data,
  ...
  // NOTE: 这里是与传统brehensam算法不同的地方，cartographer又进行了一次细化
  // step1: 这里把要更新的子图分辨率在缩小1000，用于再精细化起始点和终止点的坐标
  const double superscaled_resolution = limits.resolution() / kSubpixelScale;
  // 注意这里为更新地图重新构建一个信息结构，地图的limits.max()没有变化
  const MapLimits superscaled_limits(
      superscaled_resolution, limits.max(),
      CellLimits(limits.cell_limits().num_x_cells * kSubpixelScale,
                 limits.cell_limits().num_y_cells * kSubpixelScale));
  // step2: 计算雷达原点在地图中的像素坐标, 作为画线的起始坐标
  ...
  // step3: 更新所有hit点的概率值，直接调用ApplyLookupTable
  for (const sensor::RangefinderPoint& hit : range_data.returns) {
    // 计算hit点在地图中的像素坐标, 作为画线的终止点坐标
    ends.push_back(superscaled_limits.GetCellIndex(hit.position.head<2>()));
    // 更新hit点的栅格值（这里除kSubpixelScale是为了平衡上面的相除
  ...
  // Now add the misses.
  // step4: miss点不能像hit点一样直接用坐标更新，必须用brehensam算法更新
  ...
  // step5: 更新无反馈的雷达射线，借用miss模型
  // QUES: 解决了为什么miss点（雷达射出无穷大点），实际测量值给了个特别近的测距信息，实际含义是这个距离内不含任何障碍物
