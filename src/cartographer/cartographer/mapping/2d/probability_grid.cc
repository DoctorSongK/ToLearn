...
// core: 整个地图的扩张过程总结，首先利用Grid2D::GrowLimits完成原有地图的二倍扩张，后利用ComputeCroppedGrid函数将没有用到的点云部分进行切割
// 根据bounding_box对栅格地图进行裁剪到正好包含点云
std::unique_ptr<Grid2D> ProbabilityGrid::ComputeCroppedGrid() const {
  Eigen::Array2i offset;
  CellLimits cell_limits;
  // 根据bounding_box对栅格地图进行裁剪,到这里cell_limits就不再是正方形尺寸啦，cell_limits中的x、y个数不再相同
  ...
  for (const Eigen::Array2i& xy_index : XYIndexRangeIterator(cell_limits)) {
    // xy_index是新地图像素坐标系，xy_index+offset是老地图像素坐标系，所以这里表的访问也是之前的表
