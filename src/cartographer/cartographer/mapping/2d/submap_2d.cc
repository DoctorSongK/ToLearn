...
void Submap2D::ToResponseProto(
  ...
  // note: const在*后边,
  // 指针指向的地址不能变,而内存单元中的内容可变，下面为指针常量，即指针地址不发生改变
...
void Submap2D::InsertRangeData(
  ...
  // 将雷达数据写到栅格地图中
  // unique_ptr::get()
  // 返回原有保存的指针（并不释放指针），若智能指针释放了其对象，返回的指针多指向的对象也就消失啦
...
std::unique_ptr<GridInterface> ActiveSubmaps2D::CreateGrid(
  ...
  switch (options_.grid_options_2d().grid_type()) {
    // 概率栅格地图
    // note:子图的原点在传入时为激光雷达坐标，且子图原点位于中心，然后以初始长为100构建正方形，左上角为子图最大坐标值
