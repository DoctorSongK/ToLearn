...
// 插入雷达数据结束
// note: 在点云插入后，减去更新时加上的kUpdateMarker, 更新时+kUpdateMarker，是为了保证一帧点云下，栅格只能被一次更新；
// 待更新完毕后又减去，为下次更新做准备
...
void Grid2D::ComputeCroppedLimits(Eigen::Array2i* const offset,
  ...
  *offset = known_cells_box_.min().array();
  // extend(Vector2i(2, 1)) 
  // extend(Vector2i(6, 3)) 
  // extend(Vector2i(6, 7))      => output  min().array() = (2,1) max().array() = (6,7)
  // extend(Vector2i(2, 6)) 
...
void Grid2D::GrowLimits(const Eigen::Vector2f& point,
  ...
  while (!limits_.Contains(limits_.GetCellIndex(point))) {
    ...
    // 将xy扩大至2倍, 中心点不变, 向四周扩大；max在增加时，仅是在以前基础上叠加offset，因此认为max是全局坐标系
    /**     
     *      (像素坐标系)
     *      new _________________
     *      |    （像素坐标系)     |            其中o_x代表x_offset值，new代表新生成的像素坐标系原点，old为地图生长前像素坐标系原点；
     *      |    old _______     |            可以注意到像素坐标系和地图坐标系相反，在地图坐标系中y方向的增量在像素坐标系为x方向增量，可解释下方为什么y_offset在前；
     *      |    |         |     |            另：新成长地图的max点（地图坐标系下）是基于old时增量计算得到，这里先认为max中的值都是整张地图的绝对坐标
     *      |o_x |         | o_x |
     *      |    |         |     |            下述代码的实际操作就是将老的矩形区域基于中心点向四周扩大offset,并将old像素坐标系中的点转换到new坐标系下
     *      |    V<........|     |
     *      |              O     |
     *      V_______（地图坐标系)__|
     * 
    ...
    const MapLimits new_limits(
        limits_.resolution(),
        limits_.max() +
            limits_.resolution() * Eigen::Vector2d(y_offset, x_offset),    // NOTE: 为什么这里是y_offset在前？因为该部分是将像素坐标系增量变换到了地图坐标系下
        CellLimits(2 * limits_.cell_limits().num_x_cells,
                   2 * limits_.cell_limits().num_y_cells));

    /**下列计算的依据为ToFlatIndex（二维转一维公式： index = y * x_num + x） 坐标系均为像素坐标系
    
     * 若发生上述地图扩张，上述公式中的y变更为y+offset_y, x变更为x+offset_x, x_num变更为new_x_num
     * 因此就有了下述stride 、 offset计算
     */
    ...
    limits_ = new_limits;
    if (!known_cells_box_.isEmpty()) {
      // 将known_cells_box_的x与y进行平移到老地图的范围上，即将known_cells_box_中存放的像素点坐标向右下平移，保证坐标系一致
