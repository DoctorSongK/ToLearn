...
/// @brief 检查是否是纯定位模式,支持2种纯定位的参数名字,老参数已经弃用,会报警告但程序不会终止
 ...
 * 操作步骤：
 *        a: 检查当前轨迹id，并将当前trajectory_builders这个容器中的size当做轨迹id
 *        b: 生成运动滤波器，当时间、位姿、姿态均小于阈值的时候认为节点相似
 * 
 * @param[in] expected_sensor_ids 所有需要的topic的名字的集合
 * @param[in] trajectory_options 轨迹的参数配置
 * @param[in] local_slam_result_callback 需要传入的回调函数，实际意义是将carto中局部匹配的数据传输给carto_ros用于显示
...
int MapBuilder::AddTrajectoryBuilder(
  ...
  // a. id是从零开始的, 所以新trajectory_id就是trajectory_builders_的size()
  const int trajectory_id = trajectory_builders_.size();

  // b. 运动过滤器, 运动太小没必要进行更新，注意这为后端的运动滤波器，在local_trajectory_builder中同样也有运动滤波器
  // 配置文件中没有 pose_graph_odometry_motion_filte
  // c++11: std::optional 常用于解释可能存在或者不存在的东西，增加代码可读性
  ...
  // c. 构建LocalTrajectoryBuilder（即前端内容）
  ...
  if (trajectory_options.has_initial_trajectory_pose()) {
    ...
    // 在位姿图中设置初始位姿
    // TODO: 代码从这里读取了初始位姿，那么说当完成重定位时，也需要在这将初始位姿完成赋值操作。
...
std::map<int, int> MapBuilder::LoadState(
  ...
  // 更新约束中节点与子图的轨迹id，例如：以前id是1，上一步中执行了更新，id发生变化，所以将以前proto中约束对应的轨迹更新为最新的
