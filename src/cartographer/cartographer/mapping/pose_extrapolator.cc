...
void PoseExtrapolator::AddPose(const common::Time time,
  ...
  // 在timed_pose_queue_中保存pose，更新scan_match最新位姿
  ...
  // 将imu_tracker_更新到time最新时刻(其实就是外推作用，减少计算add_pose时产生的延时)
  ...
  // 完成其余两个imu_tracker的校准，都和CSM得出的结果一致
  // 用于根据里程计数据计算线速度时姿态的预测
  // core: 在这里将CSM的结果又重新赋予odom和imu，完成姿态更新（只不过这里的姿态并不是scan_match计算纠正的，而是历史计算的）
  // QUES: 用add_pose添加姿态时，未对imu_trackers进行校准，导致后面的姿态变化量也是按照之间未校准计算
...
// 修剪imu的数据队列,丢掉过时的imu数据（即使imu数据过老，也会在imu接收队列中保留1个imu数据）
...
// 修剪odom的数据队列,丢掉过时的odom数据（即使odom数据过老，也会在odom接收队列中保留两个odom数据）
 ...
 *        该函数负责更新PoseExtrapolator类中所有ImuTracker类型的实例中的orientation，gravity_vector_，time，和imu_angular_velocity_;
 *        此时得到的Imu_linear_Acceleration不会被用来估计线性平移，仅被用来更新gravity_vector_；
 *        当不使用IMU数据时，angular_velocity_from_poses或angular_velocity_from_odometry数据被传递给imu_tracker
...
void PoseExtrapolator::AdvanceImuTracker(const common::Time time,
  ...
  // note: 这里是对不使用IMU数据的处理，此时重力方向不变，角速度为from_pose或from_odom
  ...
  // 也就是说这里仅对大于imu_tracker.time()，小于给定时间的IMU数据进行处理
...
// 计算从imu_tracker到time时刻的姿态变化量，即完成相对姿态外推
