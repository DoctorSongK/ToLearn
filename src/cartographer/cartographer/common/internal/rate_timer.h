...
class RateTimer {
 ...
 private:
  ...
  std::string DeltasDebugString() const {
    ...
    // https://blog.csdn.net/qq_21567767/article/details/82023752
    // c++11: std::accumulate(InputIterator first, InputIterator last, T init, BinaryOperation binary_op), 作用是将范围中的所有值累加[first,last)到init的结果；
    // 参数：first、last是指输入到序列中的初始位置和最终位置，使用范围是[first,last)
    // 参数：init是累加器的初始值
    // 参数：binary_op，用于完成其他操作，其中将init原值作为输入param1，所得累加值作为输入param2
