...
class BlockingQueue {
 public:
  // QUES: 经调试发现这个缓冲队列的大小就是0，也就是说容器中只能存放一个数据，这样的话高频IMU的优势也就没用上，
  // 根据这个机制，必须是所有传感器队列都有数据了才能分发，所以所有数据会相对滞后，如果是匀速环境下影响不大；但传感器优势不明显
  ...
  /// @brief 该函数作用与pop相同，但是peek仅仅是读取队列中front值，不做抛出处理
  template <typename R>
  ...
  // 返回第一个数据的指针, 如果队列为空则返回nullptr; 但是这个还和pop有所不同，没有条件变量
  template <typename R>
  const R* Peek() {
    absl::MutexLock lock(&mutex_);
    if (deque_.empty()) {
 ...
 private:
  ...
  std::deque<T> deque_ GUARDED_BY(mutex_);      // 缓冲区
};
