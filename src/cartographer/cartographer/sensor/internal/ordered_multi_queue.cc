...
void OrderedMultiQueue::Add(const QueueKey& queue_key,
  ...
  // 传感器数据的分发处理
  // QUES: 看代码发现，cartographer算法是生产一个消费一个，若不生产也就不进行消费
  Dispatch();
}
 ...
 */
// QUES: 问题是如果传感器过于高频（如我们所用imu为200hz，与雷达差距过高，按照下面算法所述，当只有imu有数据是，dispatch是无法进行的，需实验验证）
  ...
  while (true) {
    ...
    */
    ...
    for (auto it = queues_.begin(); it != queues_.end();) {
      ...
      if (data == nullptr) {
        // 如果队列已经处于finished状态了, 就删掉这个队列，并重新搜索其他队列
    ...
    // 需要注意的是common_start_time仅在轨迹开始时进行更新，有此解释方便下面理解
