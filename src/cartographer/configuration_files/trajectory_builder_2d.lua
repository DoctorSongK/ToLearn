...
TRAJECTORY_BUILDER_2D = {
  ...
  -- 通过保证体素单元内的min_num_points最小点的个数来动态调整单元边长
  ...
  -- 计算复杂度高 , 但是很鲁棒 , 在odom或者imu不准时依然能达到很好的效果，这里的暴力匹配是一种粗匹配，经计算之后还需传给下面的ceres_scan_matcher
  use_online_correlative_scan_matching = false,
  real_time_correlative_scan_matcher = {
    linear_search_window = 0.1,             -- 线性搜索窗口的大小（x、y方向上的窗口大小）
  ...
  ceres_scan_matcher = {
    ...
    ceres_solver_options = {
      use_nonmonotonic_steps = false, --true 允许ceres计算过程中可以出现cost短暂上浮的一种情况，从而实现跳过局部最优解，获得逼近最优解
  ...
  imu_gravity_time_constant = 10., --imu时间常数
