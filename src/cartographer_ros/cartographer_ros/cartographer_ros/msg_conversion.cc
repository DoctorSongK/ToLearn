...
// 在这里将ros中的激光点云转换为具备时间增量、转换为激光坐标系下（x,y,z）的激光点云信息
// 这里认为ros接收到激光雷达数据的时间戳是第一个激光点数据的，所以后面处理过程中将该时间戳加上了总累加量当做该帧时间戳（最后一个点）
template <typename LaserMessageType>
std::tuple<PointCloudWithIntensities, ::cartographer::common::Time>
LaserScanToPointCloudWithIntensities(const LaserMessageType& msg) {
  CHECK_GE(msg.range_min, 0.f);
  CHECK_GE(msg.range_max, msg.range_min);
  // 这里的angle_increment在大于0时，要求angle_max > angle_min，否则需要反过来
  ...
  for (size_t i = 0; i < msg.ranges.size(); ++i) {
    ...
    // 主要是验证超声波雷达是否为空，对于laserscan来讲并无较大意义
    if (HasEcho(echoes)) {
      ...
      if (msg.range_min <= first_echo && first_echo <= msg.range_max) {
        ...
        // 这里给每个激光点附上时间信息，并将极坐标转换为直角坐标系
        // QUES: 不是说最后一个点的时间增量为0，其他点的时间增量为负值吗(在后面完成时间增量更改)；还有以向量形式进行转换是怎么做到的
...
// 检查点云是否存在 field_name 字段，主要针对3d点云中的field
...
ToPointCloudWithIntensities(const sensor_msgs::LaserScan& msg) {
  // c++11: 可以显示指定模版如函数名<int>形式，也可以实现参数类型的自动推导
 ...
 * 
 * @param[in] latitude 纬度数据 （单位为度，需转换为弧度）
 * @param[in] longitude 经度数据（单位为度，需转换为弧度）
 * @return cartographer::transform::Rigid3d 计算第一帧GPS数据指向ECEF坐标系下原点的坐标变换
 */

// Eigen知识补充：Eigen::AngleAxisd(旋转角alpha,旋转轴Vector3d(x,y,z))
cartographer::transform::Rigid3d ComputeLocalFrameFromLatLong(
  ...
  // ECEF坐标系下原点的转换的反变换（所以加了负号，即坐标原点相对于当前经纬度）
