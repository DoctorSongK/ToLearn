...
std::unique_ptr<carto::sensor::OdometryData> SensorBridge::ToOdometryData(
  ...
  // 按照配置来看，获得base_link相对于imu_link(tracking frame)
...
// 调用trajectory_builder_的AddSensorData进行数据的处理(这里提及的trajectory_build指的是collated_trajectory_builder)
void SensorBridge::HandleOdometryMessage(
  ...
  if (odometry_data != nullptr) {
    // tag: 这个trajectory_builder_是谁, 讲map_builder时候再揭晓(实际这个地方是collated_trajectory_builder)
...
void SensorBridge::HandleNavSatFixMessage(
  ...
  // 计算第一帧GPS数据指向ECEF坐标系下原点的坐标变换, 用这个坐标变换乘以之后的GPS数据
...
std::unique_ptr<carto::sensor::ImuData> SensorBridge::ToImuData(
  ...
  // 推荐将imu的坐标系当做tracking frame，cartographer这里指明：如果将imu_frame 与 tracking_frame相差较远，
  // 将线加速度转换至tracking_frame时会导致误差增大
  CHECK(sensor_to_tracking->translation().norm() < 1e-5)
      << "The IMU frame must be colocated with the tracking frame. "
         "Transforming linear acceleration into the tracking frame will "
         "otherwise be imprecise.";
  // 进行坐标系的转换，因为上述已经要求距离不能太远，故下面操作仅进行了旋转操作
...
void SensorBridge::HandleLaserScan(
  ...
  // param: num_subdivisions_per_laser_scan 将一帧雷达分为几份
  for (int i = 0; i != num_subdivisions_per_laser_scan_; ++i) {
    ...
    const double time_to_subdivision_end = subdivision.back().time;   // 此时最后一个点的时间戳为负值
...
void SensorBridge::HandleRangefinder(
  ...
  if (sensor_to_tracking != nullptr) {
    // QUES: 这个地方应该是带有强度信息的，李想说没有，需要验证一下
    // NOTE: 此时传入sensor_bridge的是collated_trajectory_builder，所以下面的传感器调用也是如此
    trajectory_builder_->AddSensorData(
