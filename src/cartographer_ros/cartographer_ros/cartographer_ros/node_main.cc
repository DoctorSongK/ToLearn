...
void Run() {
  ...
  // c++11: std::tie()函数可以将变量连接到一个给定的tuple（元组类型）上,生成一个元组类型全是引用的tuple
  // c++11: 元组类型特性：1.是一种不可改变顺序、增删的列表类型 2.元组可由一种或多种合法数据类型组成（"as",1,[2,3]）

  // 根据Lua配置文件中的内容, 为node_options, trajectory_options 赋值
  std::tie(node_options, trajectory_options) =
      LoadOptions(FLAGS_configuration_directory, FLAGS_configuration_basename);

  //接口：完成与cartographer的api连接
  ...
  //单线程的，就是所有的callback是按照顺序执行的，也就是说传感器数据接收需按照一定的顺序执行，不是同时进行的
  ::ros::spin();
...
int main(int argc, char** argv) {
  ...
  // note: 初始化glog库
  // study: argc是命令行总的参数个数，argv是argc个参数，其中第0个参数是程序全名，后续参数是命令行后面跟的用户输入参数
