...
// 那这里返回的坐标变换的z是 -0.1
/**
 * @brief 获取frame_id到tracking_frame_的坐标转换
 * 
 * @param[in] time 
 * @param[in] frame_id 
 * @return std::unique_ptr<::cartographer::transform::Rigid3d> 
 */
std::unique_ptr<::cartographer::transform::Rigid3d> TfBridge::LookupToTracking(
  ...
  // ros 知识补充：void Transformer:: lookuptransform (const std::string & target_frame,const std::string & source_frame, const ros::Time & Time, StampedTransform & transform)
  // 这里的target_frame 为parent_frame;source_frame 为child_frame
  try {
