...
void MapBuilderBridge::LoadState(const std::string& state_filename,
  ...
  // c++11: substr(size_type_Off = 0, size_type_Count = npos) 主要功能是复制子字符串，要求从指定位置开始，并具有指定的长度。
  // 形式：s.substr(pos,len)
  // 返回值：string，包含s中从pos开始的len个字符的拷贝（pos的默认值为0，len的默认值是s.size() - pos，即不加参数会拷贝整个s）
  // 异常：若pos的值超过了string的大小，则substr函数会抛出一个out_of_range异常；若pos+n的值超过了string的大小，则substr会调整n的值，只拷贝到string的末尾
  // 如果没有指定长度_Count或_Count_Off超出了源字符串的长度，则子字符串将延续到源字符串的结尾
  ...
  // 解析数据,将pbstream中的约束、传感器、子图、前端等数据传到了pose_grapher中
...
int MapBuilderBridge::AddTrajectory(
  ...
  // Step: 2 为这个新轨迹 添加一个SensorBridge(实际作用：将)
...
// 获取local坐标系下的TrajectoryData(包含localslam,以及local相对于global的坐标转换)
