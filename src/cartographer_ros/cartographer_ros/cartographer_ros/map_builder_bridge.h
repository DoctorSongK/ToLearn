 ...
 */
  ...
  /**
   * @brief LocalTrajectoryData内容：累计所有雷达数据，每帧雷达估计的位姿、时间，local_frame到global_frame的坐标转换
   * 
   */
  ...
  // MapBuilderBridge完成与cartographer算法中map_builder的连接，其接口大多也和map_builder一样
  MapBuilderBridge(
 ...
 private:
  ...
  std::unordered_map<int,
      ...
      local_slam_data_ GUARDED_BY(mutex_);                                      // 记录localslam的数据，int为轨迹index
  ...
  std::unordered_map<int, std::unique_ptr<SensorBridge>> sensor_bridges_;       // 存放sensor_bridge，int为轨迹index
