...
NodeOptions CreateNodeOptions(
  ...
  options.map_builder_options =
      ::cartographer::mapping::CreateMapBuilderOptions(
          lua_parameter_dictionary->GetDictionary("map_builder").get()); //.get()是智能指针获得值的时候用的
  ...
  // 针对默认值的处理，下列publish_to_tf、publish_tracked_pose等等已经在结构体NodeOption中赋值，所以先去访问一下lua中
  // 有没有这个参数赋值，如果有的话再覆盖掉
