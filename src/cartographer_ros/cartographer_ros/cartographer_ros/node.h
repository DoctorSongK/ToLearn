...
class Node {
 ...
 private:
  ...
  struct TrajectorySensorSamplers {
    // param: lua中rangefinder_sampling_ratio odometry_sampling_ratio fixed_frame_pose_sampling_ratio imu_sampling_ratio landmarks_sampling_ratio参数在此处被应用，若为1则全部数据都应用，若不为1则数据按照指定频率采用
  ...
  // These are keyed with 'trajectory_id'.S
  std::map<int, ::cartographer::mapping::PoseExtrapolator> extrapolators_;      // 存放姿势外插器,first为trajectory_index
  std::map<int, ::ros::Time> last_published_tf_stamps_;                         // 存放最新发布tf的时间戳，first为trajectory_index
  std::unordered_map<int, TrajectorySensorSamplers> sensor_samplers_;           // 存放传感器采样频率，first为trajectory_index，用于控制传感器频率滤波
  std::unordered_map<int, std::vector<Subscriber>> subscribers_;                // 存放不同轨迹中的所有订阅者，first为trajectory_index
  std::unordered_set<std::string> subscribed_topics_;                           // 用于存放订阅的各类话题名称
  std::unordered_set<int> trajectories_scheduled_for_finish_;

  // We have to keep the timer handles of ::ros::WallTimers around, otherwise
  // they do not fire.
  // c++11: 墙钟时间和CPU时间的区别： 
  // wall_clock_time:从进程开始运行到结束，时钟走过的时间，这其中包含了进程阻塞、等待状态的时间
  // 用户CPU时间：就是用户的进程获得了CPU资源以后，在用户态执行的时间
  // 系统CPU时间：用户进程获得了CPU资源以后，再内核态的执行时间
  std::vector<::ros::WallTimer> wall_timers_;       //基于墙钟时间的计时器，完成所有的定时操作
