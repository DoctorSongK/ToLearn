 ...
 */
// NOTE: 注意这里的函数指针是作为一种形参，如void (Node:*handler)(int, const std::string&, const typename MessageType::ConstPtr&)
// 如上的实际意义是handler是一种函数指针形式，跟int double一样是一种Node类中的一种函数指针类型。
...
::ros::Subscriber SubscribeWithHandler(
  ...
  return node_handle->subscribe<MessageType>(
      ...
      boost::function<void(const typename MessageType::ConstPtr&)>(
          // c++11: lambda表达式
          // 为什么要用lambda表达式，是因为话题函数中的形参只能为对应的msg格式，若想用多参数，就得这么干
...
// TODO: 等到看完node之后，再转过头来看map_builder_bridge
Node::Node(
  ...
  // 发布SubmapList,只有子图index、位姿、轨迹id等信息，没有实际地图数据
...
/**
 * @brief 获取对应id轨迹的 索引为submap_index 的submap(子图对应的栅格数据等信息)
 *
...
void Node::AddExtrapolator(const int trajectory_id,
  ...
  // param: lua中的imu_gravity_time_constant参数在此被应用 
...
void Node::PublishLocalTrajectoryData(const ::ros::TimerEvent& timer_event) {
  ...
  for (const auto& entry : map_builder_bridge_.GetLocalTrajectoryData()) {
    ...
    if (trajectory_data.local_slam_data->time !=
      ...
      if (scan_matched_point_cloud_publisher_.getNumSubscribers() > 0) {
        ...
        scan_matched_point_cloud_publisher_.publish(ToPointCloud2Message(
            ...
            // QUES: 不清楚这里的local_to_map具体所指（已解决：实际这边的point_cloud为local坐标系中被击中的激光点云）
    ...
    // 防止同一时间戳的tf重复发布； tf_buffer将对具有相同时间戳的重复转换发出警告
    ...
    // 获取tracking_frame在local_frame中的坐标（从extrapolator或local_slam_data中获取）
    const Rigid3d tracking_to_local_3d =
        node_options_.use_pose_extrapolator
            ? extrapolator.ExtrapolatePose(now)
            : trajectory_data.local_slam_data->local_pose;
    
    // 获取当前位姿在local坐标系下的坐标（2d）
    const Rigid3d tracking_to_local = [&] {
      // 是否将变换投影到平面上
      // param: lua中publish_frame_projected_to_2d，用于确定是否将变换投影到平面
    ...
    if (trajectory_data.published_to_tracking != nullptr) {
      if (node_options_.publish_to_tf) {
        ...
        // param: lua中参数provide_odom_frame，是否由cartographer提供odom坐标系，若提供则发布map_frame -> odom -> published_frame
      ...
      // param: 该参数未在lua中体现，程序中默认写入false，即不发布此位姿
      if (node_options_.publish_tracked_pose) {
...
Node::ComputeExpectedSensorIds(const TrajectoryOptions& options) const {
  ...
  */
  ...
  // param: lua中num_laser_scans num_multi_echo_laser_scans num_point_clouds等参数将在此处应用
  ...
  // param: lua中use_nav_sat参数在此使用，用于是否订阅gps信息，且仅能订阅一个
...
int Node::AddTrajectory(const TrajectoryOptions& options) {
  ...
  // step1: 调用map_builder_bridge的AddTrajectory, 添加一个轨迹，同时为此轨迹添加SensorBridge
...
bool Node::HandleStartTrajectory(
  ...
  // 如果给定了一个初始位姿
  // TODO: 这里的初始位姿是不是可以在定位的时候进行应用，后期可以试一下
  ...
  // 检查TrajectoryOptions是否存在2d或者3d轨迹的配置信息，在该options中必须有2d或3d配置才能执行
  if (!ValidateTrajectoryOptions(trajectory_options)) {
    response.status.message = "Invalid trajectory options.";
    LOG(ERROR) << response.status.message;
    response.status.code = cartographer_ros_msgs::StatusCode::INVALID_ARGUMENT;
  }
  // 检查topic名字是否被其他轨迹使用在，如果存在重复topic的话则无法开始新轨迹（就是以前旧的轨迹，因为subscribe_topics只能在创建轨迹时写入）
...
void Node::HandleOdometryMessage(const int trajectory_id,
  ...
  // core: step1->里程计数据流向（1、经坐标转换后传入node姿势外插器中，用于rviz显示trajectory；2、经坐标转换后数据传入至collated_trajectory_builder中）
...
void Node::HandleImuMessage(const int trajectory_id,
  ...
  // extrapolators_使用里程计数据进行位姿预测
  // core: IMU数据流向（1、经坐标转换后传入姿势外插器中；2、经坐标转换后数据传入至collated_trajectory_builder中）
...
void Node::HandleLaserScanMessage(const int trajectory_id,
  ...
  absl::MutexLock lock(&mutex_);
  // 根据配置,是否将传感器数据跳过（即频率滤波）
  if (!sensor_samplers_.at(trajectory_id).rangefinder_sampler.Pulse()) {
    return;
  }
  // 在这里传感器信息被colate_trajectory_builder_中的add_sensor所添加
  // core: laser数据流向（经直角坐标系转换后数据传入至collated_trajectory_builder中）
  map_builder_bridge_.sensor_bridge(trajectory_id)
